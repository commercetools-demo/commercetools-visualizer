import { CustomFormDetailPage, CustomFormModalPage, FormModalPage, PageNotFound } from "@commercetools-frontend/application-components";
...
import { useSubscriptionDeleter, useSubscriptionFetcher, useSubscriptionKeyUpdater } from "./subscription-connectors";

type TErrors = {
    key: { missing?: boolean };
};

export const validate = (formikValues: TFormValues): FormikErrors<TFormValues> => {
...
const SubscriptionDetail: FC<Props> = ({ linkToWelcome }) => {
    ...
    const handleSubmit = useCallback(
        ...
        [subscription, intl, showApiErrorNotification, showNotification, subscriptionKeyUpdater]
    );
            ...
            {(formProps) => {
                return (
                    <CustomFormDetailPage
                        title={formProps.values?.key || intl.formatMessage(messages.subscriptionKeyLabel)}
                        onPreviousPathClick={() => history.push(linkToWelcome)}
                        formControls={
                            <>
                                <CustomFormDetailPage.FormSecondaryButton label={FormModalPage.Intl.revert} isDisabled={!formProps.isDirty} onClick={formProps.handleReset} />
                                <CustomFormDetailPage.FormPrimaryButton isDisabled={
                                    formProps.isSubmitting || !formProps.isDirty || !canManage
                                }
                                    onClick={() => formProps.submitForm()} label={FormModalPage.Intl.save} />
